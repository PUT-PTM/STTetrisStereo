...
extern const u8 sound[10516];
extern bool soundCheck;
...
void TIM5_IRQHandler(void) { //Music Loop
	if (TIM_GetITStatus(TIM5, TIM_IT_Update) != RESET) {
		...
		if (!soundCheck) {
			DAC_SetChannel1Data(DAC_Align_12b_R, music[musicPlayer]);
			musicPlayer++;

			if (musicPlayer >= 103587)
				musicPlayer = 0;

		} else {
			DAC_SetChannel1Data(DAC_Align_12b_R, sound[soundPlayer]);
			soundPlayer++;
			if (soundPlayer >= 3890) {
				soundPlayer = 0;
				soundCheck = false;
